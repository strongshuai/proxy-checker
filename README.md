...
- 仓库 TXT / JSON 链接支持服务端过滤参数，便于脚本直接拉取指定状态的代理。

仓库链接过滤示例：

```text
/api/repo/<token>.txt?grade=A,B&service=1&cf=1&ip_type=residential&country=US
/api/repo/<token>.json?grade=A&api=1&max_latency=1500
```
...
| 参数 | 说明 |
|---|---|
| `grade` / `grades` | 等级，支持逗号分隔，例如 `A,B,C` |
| `service` / `service_reachable` / `service_ok` | `1/true` 只保留服务可达，`0/false` 只保留不可达 |
| `api` / `api_reachable` / `api_ok` | `1/true` 只保留 API 域名可达 |
| `cf` / `cf_bypass` / `cf_ok` | `1/true` 只保留网页 CF 未拦截 |
| `ip_type` / `type` | `datacenter` / `residential`，也支持 `dc` / `res` |
| `country` / `countries` | 国家代码，支持逗号分隔；`country=1` 表示只保留有国家信息 |
| `target_profile` / `profile` | 检测目标，例如 `generic` / `openai` / `grok` |
| `recommended_use` / `use` | 推荐用途 |
| `max_latency` / `latency_lte` / `latency_max` | 最大延迟毫秒 |
